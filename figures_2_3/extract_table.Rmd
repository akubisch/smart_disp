...
pair_base = c("L/TA", "L/TAE", "L/S", "TA/TAE", "TA/S", "TAE/S")
...
for (i in c("_i02","_i05","_i08")) {
  for (s in c("sig0","sig125","sig25")) {
    ...
    L = c(L,round(x[1:3]*100),"--","--","--")
    TA = c(TA, round((1-x[1]-y[1])*100), "--","--", round(x[4:5]*100), "--")
    TAE = c(TAE,"--",round((1-x[2]-y[2])*100),"--",round((1-x[4]-y[4])*100),"--",round(x[6]*100))
    S = c(S,"--","--",round((1-x[3]-y[3])*100),"--",round((1-x[5:6]-y[5:6])*100))
    draw = c(draw,y*100)
  }
}

results <- data.frame(I=info,sigma=sigma,pairing=pairing,draw=draw,TA=TA,TAE=TAE,L=L,S=S)
